...
export class Controller {
    ...
    this.lastStep = new Promise((resolve) => {
      ...
      const dispatchChanges = (): void => {
        ...
        this.dispatchChangesTimeoutId = window.setTimeout(() => {
          console.log(
            `[stepFrom][c${this.__id}] CPU step produced memory[0..7]:`,
            memoryData.slice(0, 8),
          )
          ...
          console.log(`[stepFrom][c${this.__id}] dispatched CPU memory to Redux`)
  ...
  public injectRandomAt = (address: number): void => {
    const mem = store.getState(selectMemoryData)
    const next = [...mem]
    console.log(`[injectRandomAt][c${this.__id}] called with address:`, address)
    console.log(`[injectRandomAt][c${this.__id}] current mem[0..7]:`, mem.slice(0, 8))
    const value = Math.floor(Math.random() * 256) & 0xff
    next[address] = value

    console.log(`[injectRandomAt][c${this.__id}] writing value ${value} at ${address}`)

    store.dispatch(setMemoryData(next))

    if (address >= VDU_START_ADDRESS) {
      store.dispatch(setVduDataFrom(next))
    }

    const regs = store.getState(selectCpuRegisters)
    this.lastStep = Promise.resolve({
      memoryData: next,
      cpuRegisters: regs,
    })
